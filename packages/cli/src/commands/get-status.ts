...
import { CLIOptions, StatusOutput, ValidationError } from "../types";
...
import { Colors, printError, printKeyValue, printSection } from "../utils/ui";
    ...
    .action(async (options: CLIOptions) => {
      try {
        ...
        if (error instanceof ValidationError) {
          ...
          if (error.message.includes("NotFound")) {
            if (options.json) {
              console.log(JSON.stringify({
                error: "NotFound",
                message:
                  `No verification found for ${options.projectId}@${options.version}`,
              }));
            } else {
              printError(
                "Not Found",
                `No verification found for ${Colors.cyan(options.projectId!)}@${
          ...
          if (options.json) {
            console.log(
              JSON.stringify({ error: "NetworkError", message: error.message }),
            );
          } else {
          ...
          logger.error("Unexpected error in get-status command", {
            error: errorMessage,
          });
          process.exit(1);
        }
...
function displayStatus(status: StatusOutput): void {
  ...
  if (status.totalExecutors !== undefined) {
    ...
    if (status.completedExecutors !== undefined) {
      ...
      printKeyValue(
        "Completed Executors",
        `${status.completedExecutors}/${status.totalExecutors} (${progress}%)`,
      );
    }
    ...
    if (status.successfulExecutors !== undefined) {
      printKeyValue(
        ...
        status.successfulExecutors.toString(),
      );
    }
    ...
    if (status.consensusThreshold !== undefined) {
      printKeyValue(
        "Consensus Threshold",
        status.consensusThreshold.toString(),
      );
    }
