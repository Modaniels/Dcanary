...
function createLogsCommand() {
    ...
    command
        ...
        try {
            console.log();
            console.log(ui_1.Colors.bold('📋 Dcanary ICP Build Logs'));
            console.log();
            // Sample log output
            const logs = [
            ...
            displayLogs.forEach(log => {
                if (log.includes('✅')) {
                    console.log(ui_1.Colors.green(log));
                }
                else if (log.includes('❌')) {
                    console.log(ui_1.Colors.red(log));
                }
                else if (log.includes('[INFO]')) {
                    console.log(ui_1.Colors.gray(log));
                }
                else {
                    console.log(log);
                }
            ...
            console.log();
            if (options.follow) {
                console.log(ui_1.Colors.gray('👀 Following logs... (Press Ctrl+C to stop)'));
                // TODO: Implement real-time log streaming
            }
            else {
                console.log(ui_1.Colors.gray(`Showing last ${linesToShow} lines. Use --follow to watch live.`));
            }
        }
        catch (error) {
