...
export interface WebhookCanister {
    register_repository: (config: RepositoryConfig) => Promise<{ Ok: string } | { Err: any }>;
    handle_webhook: (payload: any) => Promise<{ Ok: WebhookResult } | { Err: any }>;
    get_repository_config: (repo_id: string) => Promise<{ Ok: RepositoryConfig } | { Err: any }>;
    list_repositories: () => Promise<RepositoryConfig[]>;
    update_repository_config: (repo_id: string, config: RepositoryConfig) => Promise<{ Ok: string } | { Err: any }>;
    remove_repository: (repo_id: string) => Promise<{ Ok: string } | { Err: any }>;
...
export interface VerificationCanister {
    create_verification_request: (request: VerificationRequest) => Promise<{ Ok: string } | { Err: any }>;
    get_verification_status: (request_id: string) => Promise<{ Ok: VerificationStatus } | { Err: any }>;
    submit_build_result: (request_id: string, result: BuildResult) => Promise<{ Ok: string } | { Err: any }>;
    get_build_logs: (request_id: string) => Promise<{ Ok: string[] } | { Err: any }>;
    list_verification_requests: (filter?: any) => Promise<VerificationRequest[]>;
    get_pipeline_status: (pipeline_id: string) => Promise<{ Ok: PipelineStatus } | { Err: any }>;
}
...
export interface BuildExecutorCanister {
    execute_build: (instructions: BuildInstructions) => Promise<{ Ok: BuildResult } | { Err: any }>;
    get_build_status: (build_id: string) => Promise<{ Ok: BuildStatus } | { Err: any }>;
    get_available_executors: () => Promise<ExecutorInfo[]>;
    register_executor: (executor: ExecutorConfig) => Promise<{ Ok: string } | { Err: any }>;
}
...
export class CanisterIntegrationService {
    ...
    private buildExecutorActor: ActorSubclass<BuildExecutorCanister> | null = null;
    ...
    private async initializeAgent(): Promise<void> {
        try {
            ...
            logger.info('IC Agent initialized', { network, providerUrl });
        } catch (error) {
    ...
    private async getIdentity(): Promise<SignIdentity> {
        ...
        try {
            ...
            if (await fs.pathExists(identityPath)) {
                ...
                try {
                    this.identity = (Ed25519KeyIdentity as any).fromPem?.(identityPem) || Ed25519KeyIdentity.generate();
                } catch {
                ...
                this.identity = Ed25519KeyIdentity.generate();
                logger.warn('Using anonymous identity - configure with dcanary configure --identity <path>');
            }
    ...
    private async getVerificationActor(): Promise<ActorSubclass<VerificationCanister>> {
    ...
    async removeRepository(repoId: string): Promise<boolean> {
        try {
            ...
            if ('Ok' in result) {
                ...
                logger.error('Failed to remove repository', { repoId, error: result.Err });
                return false;
            }
        } catch (error) {
    ...
    async getWebhookEvents(repoId: string, limit?: number): Promise<WebhookEvent[]> {
        try {
            const actor = await this.getWebhookActor();
    ...
    async createVerificationRequest(request: VerificationRequest): Promise<string> {
        try {
