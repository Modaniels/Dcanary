...
import { validateProjectId, validateVersion, validateCanisterId } from '../utils/helpers';
            ...
            try {
                ...
                if (!canisterId) {
                    throw new ValidationError(
                        'Build instructions canister ID is required. ' +
                ...
                spinner.start(`Adding build instructions for ${Colors.cyan(options.projectId!)}@${Colors.cyan(options.version!)}`);
                ...
                spinner.succeed(
                    `Build instructions added successfully for ${Colors.cyan(options.projectId!)}@${Colors.cyan(options.version!)}`
                );
                ...
                console.log(`  Instructions length: ${Colors.gray(instructionSet.length.toString())} characters`);
                ...
                if (error instanceof ValidationError) {
                    ...
                    logger.error('Unexpected error in add-instructions command', { error: errorMessage });
                    process.exit(1);
                }
            }
        });
...
function validateInputs(options: CLIOptions): void {
    ...
    if (!options.instructionSet && !options.file) {
        throw new ValidationError('Either --instruction-set or --file must be provided');
    }

    if (options.instructionSet && options.file) {
        throw new ValidationError('Cannot specify both --instruction-set and --file');
    }
...
function getInstructionSet(options: CLIOptions): string {
    ...
    if (options.file) {
        try {
            ...
            const errorMessage = error?.message || 'Unknown error occurred';
            throw new ValidationError(`Failed to read file ${options.file}: ${errorMessage}`);
        }
    }
