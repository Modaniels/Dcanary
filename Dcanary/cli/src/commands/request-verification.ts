...
import { CLIOptions, ValidationError, VerificationProgress } from '../types';
...
import { Colors, printSuccess, printError, Spinner, ProgressBar, printInfo } from '../utils/ui';
    ...
    command
        ...
        .action(async (options: CLIOptions & { wait?: boolean }) => {
            ...
            try {
                ...
                spinner.start(`Requesting verification for ${Colors.cyan(options.projectId!)}@${Colors.cyan(options.version!)}`);
                ...
                spinner.succeed(
                    `Verification requested for ${Colors.cyan(options.projectId!)}@${Colors.cyan(options.version!)}`
                );

                // Display initial status
                console.log();
                console.log(Colors.bold('Initial Status:'));
                console.log(`  Status: ${getStatusColor(getStatusString(verificationResult.status))}`);
                console.log(`  Total Executors: ${Colors.gray(verificationResult.total_executors.toString())}`);
                console.log(`  Consensus Threshold: ${Colors.gray(verificationResult.consensus_threshold.toString())}`);
                ...
                if (options.wait !== false) {
                    ...
                    printInfo('Use "mody get-status" to check verification progress');
                }
                ...
                if (error instanceof ValidationError) {
                    ...
                    logger.error('Unexpected error in request-verification command', { error: errorMessage });
                    process.exit(1);
                }
            }
        });
    ...
    while (true) {
        ...
        try {
            ...
            if (inCI) {
                ...
                const estimated = estimateRemainingTime(elapsedSeconds, completedExecutors, totalExecutors);
...
function displayFinalResult(result: any, projectId: string, version: string): void {
    const status = getStatusString(result.status);
    ...
    console.log(`  Completed Executors: ${Colors.gray(completedExecutors.toString())}/${Colors.gray(result.total_executors.toString())}`);
    console.log(`  Successful: ${Colors.gray(successfulExecutors.toString())}`);
    console.log(`  Failed: ${Colors.gray(failedExecutors.toString())}`);
    console.log(`  Consensus Threshold: ${Colors.gray(result.consensus_threshold.toString())}`);
    console.log(`  Matching Results: ${Colors.gray(result.matching_results.toString())}`);
