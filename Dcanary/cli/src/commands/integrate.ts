...
import { Colors, printHeader, printSuccess, printError, printWarning, Spinner } from '../utils/ui';
...
import { canisterService } from '../services/canister-integration';
...
async function deployPipelineConfigCanister(repoIdentifier: string): Promise<string> {
    try {
...
async function setupWebhookCanister(platform: string, options: any): Promise<string> {
    try {
        ...
        logger.info('Platform registered with webhook canister', { platform, webhookCanisterId });
...
async function createDefaultPipelineStages(configCanisterId: string): Promise<void> {
    try {
...
export function createIntegrateCommand(): Command {
    ...
    cmd
        ...
        .action(async (platform: string, options: any) => {
            try {
                printHeader(`Setting up ${platform.toUpperCase()} Integration with DCanary Canisters`);
                ...
                console.log(Colors.gray('All configuration and execution will happen on ICP canisters'));
                console.log();
                ...
                try {
                    ...
                    console.log(`  🔗 Webhook Canister: ${Colors.cyan(webhookCanisterId)}`);
                    console.log(`  🌐 Platform: ${Colors.yellow(platform.toUpperCase())}`);
                    
                    if (options.repo) {
                        console.log(`  📁 Repository: ${Colors.cyan(options.repo)}`);
                        console.log(`  🔗 Webhook URL: ${Colors.gray(`https://${webhookCanisterId}.ic0.app/webhook`)}`);
                    }
                    
                    console.log();
                    console.log(Colors.bold('Next Steps:'));
                    if (!options.token || !options.repo) {
                        console.log(`  1. ${Colors.gray('Connect repository:')} ${Colors.cyan(`dcanary integrate ${platform} --repo owner/repo --token <token>`)}`);
                    }
                    console.log(`  2. ${Colors.gray('Configure pipeline:')} ${Colors.cyan('dcanary pipeline configure')}`);
                    console.log(`  3. ${Colors.gray('Push to trigger build:')} ${Colors.cyan('git push origin main')}`);
                    console.log(`  4. ${Colors.gray('Monitor pipeline:')} ${Colors.cyan('dcanary status --watch')}`);
                ...
                logger.error('Integration setup failed', { error: error.message, platform });
...
async function validateCanisterConfiguration(): Promise<void> {
    ...
    if (!webhookCanisterId) {
        ...
        console.log('Configure with: ' + Colors.cyan('dcanary configure --webhook-canister <canister-id>'));
    ...
    try {
        ...
        console.log(Colors.green(`✓ Connected to webhook canister (${repos.length} repositories)`));
    } catch (error) {
...
async function promptCanisterConfiguration(): Promise<void> {
    ...
    if (!configManager.get('webhookCanisterId')) {
        questions.push({
            ...
            validate: (input: string) => input.length > 0 || 'Canister ID is required'
        });
    }
    ...
    if (await fs.pathExists(dfxPath)) {
        try {
            ...
            if (dfxConfig.canisters) {
                for (const [name, config] of Object.entries(dfxConfig.canisters as any)) {
...
async function setupRepositoryIntegration(
    ...
    try {
        ...
        console.log(`  Webhook URL: ${Colors.gray(config.webhookUrl || 'Not set')}`);
        console.log(`  Auto-deploy: ${config.autoDeploy ? Colors.green('Enabled') : Colors.yellow('Disabled')}`);
        ...
        if (config.canisterConfigs.length > 0) {
            ...
            config.canisterConfigs.forEach(canister => {
                console.log(`  • ${Colors.cyan(canister.name)} (${canister.type}) → ${canister.network}`);
            });
        }
...
async function interactiveRepositorySetup(provider: any, config: SCMIntegrationConfig): Promise<void> {
    try {
        console.log();
